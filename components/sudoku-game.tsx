...
export default function SudokuGame() {
  ...
  const given = new Set(puzzle.split('').flatMap((c, i) => c !== '.' ? [i] : []))
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      } else if (e.key === 'ArrowRight') setSelected(s => s !== null ? Math.min(80, s + 1) : 0)
      else if (e.key === 'ArrowLeft') setSelected(s => s !== null ? Math.max(0, s - 1) : 0)
      else if (e.key === 'ArrowDown') setSelected(s => s !== null ? Math.min(80, s + 9) : 0)
      else if (e.key === 'ArrowUp') setSelected(s => s !== null ? Math.max(0, s - 9) : 0)
    }
